...
const net = require("net");
const shortId = require("shortid");
const debugLog = debug_1.default("AmiTestServer");
const errorLog = debug_1.default("AmiTestServer:error");
const meta = require("../package.json");
const CRLF = "\r\n";
/**
 * AmiTestServer
 */
class AmiTestServer extends events_1.EventEmitter {
    /**
     *
     * @param clients
     * @returns {*}
    ...
    static sendToClient(clientSocket, message) {
        clientSocket.write(local_asterisk_ami_event_utils_1.default.fromObject(message));
    }
    constructor(options) {
        super();
        Object.assign(this, {
            ...
            _options: Object.assign({ authTimeout: 30000, credentials: {}, maxConnections: 50, silent: false }, (options || {})),
    ...
    _connectionHandler(clientSocket) {
        ...
        Object.assign(clientSocket, {
            _authTimer: null,
            _eventStream: new local_asterisk_ami_events_stream_1.default(),
    ...
    _authHandler(action, clientSocket) {
        ...
        if (actionName !== "login" || !this._isAttempt(action.Username, action.Secret)) {
            ...
            AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Error", Message: "Authentication failed" }, responseData));
            return;
        }
        clearTimeout(clientSocket._authTimer);
        AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Success", Message: "Authentication accepted" }, responseData));
    ...
    _amiActionHandler(action, clientSocket) {
        ...
        if (!action || !actionName) {
            AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Error", Message: "Missing action in request" }, responseData));
            return;
        }
        if (actionName === "ping") {
            AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Success", Ping: "Pong", Timestamp: Date.now() / 1000 + "000" }, (action.ActionID ? { ActionID: action.ActionID } : {})));
            return;
        }
        if (actionName !== "login" && actionName !== "logoff") {
            AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Error", Message: "Invalid/unknown command" }, responseData));
            return;
        }
        if (this._authClients[clientSocket._key]) {
            if (actionName === "logoff") {
                clearTimeout(clientSocket._authTimer);
                AmiTestServer.sendToClient(clientSocket, Object.assign({ Response: "Goodbye", Message: "Thanks for all the fish." }, (action.ActionID ? { ActionID: action.ActionID } : {})));
            }
        }
        else {
...
exports.default = AmiTestServer;
//# sourceMappingURL=AmiTestServer.js.map